...
import ts.tangames.drop_the_rope.activity.adapter.RopeItemAdapter;
...
public class RopeActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        back = (FrameLayout) findViewById(R.id.back_rope);
        root = (FrameLayout) findViewById(R.id.root_rope);

        // remplissage de la grille des cordes
        grid=(GridView) findViewById(R.id.gridview_rope);
        grid.setAdapter(new RopeItemAdapter(this));
        grid.setVerticalScrollBarEnabled(false);
        for(int i = 0; i< RopeType.values().length; i++){
            grid.getAdapter().getView(i,null,null);
        }

        header=(RelativeLayout) findViewById(R.id.header_rope);
    ...
    public void onResume(){
        ...
        back.setBackgroundDrawable(getResources().getDrawable(R.drawable.rope_activity_background));
    }

    @Override
	public void onPause(){
		super.onPause();
        Log.d("ropeActivity","onPause()");
